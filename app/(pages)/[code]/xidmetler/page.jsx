import ServicesPage from "@/app/(components)/containers/Xidmetler/ServicesPage";
...
const getData = async (params) => {
  ...
  const data_xidmet = await fetchData(params?.code, "xidmetler");
...
export default async function page({ params }) {
  ...
  return (
    <>
      ...
      <ServicesPage
        data={data_xidmet}
        service={data_translate?.service}
        service_1={data_translate?.service_1}
        readmore={data_translate?.readmore}
        news={data_translate?.news}
        params={params}
      />
