import Recommendation from "@/app/(components)/containers/Tovsiye/Recommendation";
...
const getData = async (params) => {
  ...
  const data_tovsiye = await fetchData(params?.code, "tovisye_mektublari");
  return { data_footer, data_translate, data_tovsiye };
};
...
export default async function page({ params }) {
  const { data_footer, data_translate, data_tovsiye } = await getData(params);
  ...
  return (
    <Suspense>
      ...
      <Recommendation
        data={data_tovsiye}
        tovsiye={data_translate?.tovsiye}
